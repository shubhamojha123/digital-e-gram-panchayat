...
// Add admin-specific functions here, for example:
// function manageUsers() { ... }
// function manageServices() { ... } 

// Admin Dashboard - same logic as user dashboard
// Paste the full dashboard JS logic from user-dashboard.html here, updating selectors if needed. 
...
document.addEventListener('DOMContentLoaded', function() {
    // Sidebar navigation logic (show/hide sections) - run this FIRST
    ...
    // ... profile, logout, search logic (same as user dashboard) ...

    // --- Service Management (Firestore) ---
    const servicesCardsContainer = document.getElementById('services-cards-container');
    const addServiceForm = document.getElementById('add-service-form');

    // Render all services in real time
        ...
        db.collection('services').orderBy('label').onSnapshot(snapshot => {
            ...
            snapshot.forEach(doc => {
                ...
                card.innerHTML = `
                    ...
                    <p>${data.desc || ''}</p>
                    <button class="delete-service-btn" data-id="${doc.id}" style="background:#e74c3c;color:#fff;padding:0.5rem 1.2rem;border:none;border-radius:5px;font-weight:600;font-size:1rem;cursor:pointer;margin-top:1rem;">Delete</button>
                `;
    ...
    if (addServiceForm) {
        addServiceForm.addEventListener('submit', function(e) {
            ...
            if (!label || !desc || !icon) return;
            const key = label.toLowerCase().replace(/[^a-z0-9]+/g, ' ').trim();
            db.collection('services').add({ key, label, desc, icon }).then(() => {
    ...
    function renderStatus() {
        if (window._adminStatusUnsub) window._adminStatusUnsub();
        window._adminStatusUnsub = db.collection('applications').orderBy('appliedAt', 'desc').onSnapshot(snapshot => {
